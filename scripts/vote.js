...
// ===================== CONFIG =====================
const VOTER_DB_FILE = path.join(__dirname, "../data/voter_data_for_db_10.json");
const VOTER_SECRETS_FILE = path.join(__dirname, "../data/voter_secrets_for_script_10.json");
const DKG_PUBLIC_KEY_PATH = path.join(__dirname, "../data/dkgKeys/public_key.json"); // 👈 Đường dẫn mới

const WASM_PATH = path.join(__dirname, "../circuits/build/VoteProofCombined/VoteProofCombined_js/VoteProofCombined.wasm");
const ZKEY_PATH = path.join(__dirname, "../circuits/build/VoteProofCombined/VoteProofCombined.zkey");
const VKEY_PATH = path.join(__dirname, "../circuits/build/VoteProofCombined/VoteProofCombined_vkey.json");

const ELECTION_ID = "ELC2024";
const NUM_CANDIDATES = 2;
const VOTES_TO_SIMULATE = 5;
const DELAY_MS = 150;
...
// ===================== MAIN =====================
async function main() {
  // 0️⃣ Đọc Public Key từ file DKG
  if (!fs.existsSync(DKG_PUBLIC_KEY_PATH)) {
    throw new Error("❌ Không tìm thấy file public_key.json. Vui lòng chạy script DKG trước!");
  }
  const dkgKeys = JSON.parse(fs.readFileSync(DKG_PUBLIC_KEY_PATH, "utf8"));
  const PKx = BigInt(dkgKeys.x);
  const PKy = BigInt(dkgKeys.y);
  console.log(`📡 Using System PK: [${PKx.toString().slice(0, 10)}..., ${PKy.toString().slice(0, 10)}...]`);
  ...
  const dbMap = new Map(voterDb.map((v) => [String(v.hashed_key), v]));
  const eligible = voterSecrets.filter((s) => dbMap.has(String(s.hashed_key)));
  ...
  for (let i = 0; i < Math.min(VOTES_TO_SIMULATE, eligible.length); i++) {
    ...
    console.log(`🗳  Voter ${i+1} is casting vote for candidate ${choice}...`);
    ...
    try {
      ...
      const res = await verifyValidVoteLocal(proof, publicSignals, voteData);
      ...
      if (res.EC === 0) console.log(`✅ Vote submitted! TX: ${res.txHash}`);

    } catch (e) {
      console.error(`❌ Vote failed for voter ${i}:`, e.message);
      appendVote({
        election_id: ELECTION_ID,
        hashed_key: String(s.hashed_key),
        error: String(e.stack || e),
